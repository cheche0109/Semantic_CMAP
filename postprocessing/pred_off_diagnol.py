...
def find_sheets(element, matrix):
    position_coord = []
    for i in range(len(matrix)):
        for j in range(len(matrix[i])):
              if matrix[i][j] == element:
                  position_coord.append([i,j])
...
def main():
    ...
    for i in range(len(img_list)):
        ...
        for i in range(final_pred[0].size):
            for j in range(final_pred[0].size):
                if i > j:
                    # upper right
                    if final_pred[i][j] == 3:
                        pred = 'A'
                    elif final_pred[i][j] == 4:
                        pred = 'P'
                    else:
                        pred = '-'
                    with open('./Prediction_test_top/'+ pro_name + "_pred.txt", 'a') as f:
                        f.write(pro_name + ',' + str(i) + '|' + str(j) +  ',' + pred + ',' + str(prediction[0][i][j]) + ','  + str(prediction[1][i][j]) + ',' + str(prediction[2][i][j]) + ',' + str(prediction[3][i][j]) + ',' + str(prediction[4][i][j])+ '\n')
                elif i< j:
                    if final_pred[i][j] == 3:
                        pred = 'A'
                    elif final_pred[i][j] == 4:
                        ...
                        pred = '-'
                    with open('./Prediction_test_bottom/'+ pro_name + "_pred.txt", 'a') as f:
                        f.write(pro_name + ',' + str(i) + '|' + str(j) + ',' + pred + ',' + str(prediction[0][i][j]) + ','  + str(prediction[1][i][j]) + ',' + str(prediction[2][i][j]) + ',' + str(prediction[3][i][j])+ ',' + str(prediction[4][i][j])+ '\n')
